                                    ...
                                    std::vector<std::unordered_set<int>> &liveOut)
{
    ...
    while (changed)
    {
        ...
        for (int bi = (int)n - 1; bi >= 0; --bi)
        {
            ...
            {
                liveOut[(size_t)bi] = std::move(outSet);
                changed = true;
            }
            if (inSet != liveIn[(size_t)bi])
            {
                liveIn[(size_t)bi] = std::move(inSet);
                changed = true;
            }
        }
    }
...
AllocResult RegAllocator::allocate(const FunctionIR &f)
{
    ...
    for (auto &cur : intervals)
    {
        ...
        if (chosen != PhysReg::NONE)
        {
            ...
            if (idx < 0)
                throw std::runtime_error("regIndex(chosen) < 0");
